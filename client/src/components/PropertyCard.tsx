          ...
          {property.managerName && (
            <div className="absolute top-3 end-14 z-10 flex items-center gap-1.5 bg-white/90 dark:bg-black/60 backdrop-blur-sm rounded-full ps-2 pe-1 py-0.5 shadow-md">
              ...
              {property.managerPhotoUrl ? (
                <img loading="lazy" src={normalizeImageUrl(property.managerPhotoUrl)} alt="" className="w-6 h-6 rounded-full object-cover border border-[#3ECFC0]/30" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; (e.target as HTMLImageElement).nextElementSibling?.classList.remove('hidden'); }} />
              ) : null}
